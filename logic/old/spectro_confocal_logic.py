...
class ConfocalLogic(GenericLogic):
    ...
    # andorcamera = Connector(interface='EmptyInterface')
    # shamrock = Connector(interface='EmptyInterface')
    ...
    def on_activate(self):
        ...
        # self._andor = self.andorcamera()
        # self._shamrock = self.shamrock()
...
#            time.sleep(0.01)
    ...
    def start_scanner_spectro(self, step_size, resol):
        """Setting up the scanner device and starts the scanning procedure with serpentine path and
        one spectrum taken at each position
        :param step_size: in um
        :param resol: in pixel
        :return:

        TODO Add the parameters on the GUI
        """

        #self._andor.set_acquisition_mode(2)
        #self._andor.set_number_accumulations(60)

        self.xy_coord = np.zeros([resol])
        for i in range(0, resol):
            self.xy_coord[i] = step_size * i

        for i in range(len(self.xy_coord)):
            if (i % 2) == 0:
                for j in (self.xy_coord):
                    self.set_position('move', x=self.xy_coord[i]*1e-6, y=j*1e-6, z=0)
                    time.sleep(1)
            else:
                for j in reversed(self.xy_coord):
                    self.set_position('move', x=self.xy_coord[i]*1e-6, y=j*1e-6, z=0)
                    time.sleep(1)

        self.set_position('move', x=0, y=0, z=0)
        ...
        try:
            ...
            if self._scan_counter >= np.size(self._image_vert_axis):
                if not self.permanent_scan:
                    # move from the end-of-scan position to the crosshair position,
                    # which is saved in [_current_x, _current_y, _current_z].
                    # make a line:
                    rs = self.return_slowness
                    lsx = np.linspace(image[-1, 0, 0], self._current_x, rs)
                    lsy = np.linspace(image[-1, 0, 1], self._current_y, rs)
                    lsz = np.linspace(image[-1, 0, 2], self._current_z, rs)
                    if n_ch <= 3:
                        start_line = np.vstack([lsx, lsy, lsz][0:n_ch])
                    else:
                        start_line = np.vstack(
                            [lsx, lsy, lsz, np.ones(lsx.shape) * self._current_a])
                    # move to the crosshair position, counts are thrown away
