...
third analog output channel.  It was originally written for
scanning AOM voltage, but it can be used to control any parameter
in the experiment that is voltage controlled.  The hardware
range is typically -10 to +10 V.
For channel change, look for WARNING in the _generate_ramp method
...
class PowerScannerLogic(GenericLogic):
    ...
    def _generate_ramp(self, voltage1, voltage2, speed):
        ...
        scan_line = np.vstack((
            ...
            ramp,
            np.ones((len(ramp), )) * spatial_pos[3]
            ))
    ...
    def save_data(self, tag=None, colorscale_range=None, percentile_range=None):
        ...
        filepath = self._save_logic.get_path_for_module(module_name='PowerScanning')
