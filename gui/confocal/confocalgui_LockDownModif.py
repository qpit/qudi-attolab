...
class ConfocalGui(GUIBase):
    ...
    def initMainUI(self):
        ...
        *.ui file and configures the event handling between the modules.
        ...
        num_of_points_a = (self._scanning_logic.a_range[1] - self._scanning_logic.a_range[0]) / self.slider_res
        ...
        self._mw.a_SliderWidget.setRange(0, num_of_points_a)
        ...
        self._mw.a_current_InputWidget.setRange(self._scanning_logic.a_range[0],
                                                self._scanning_logic.a_range[1])
        ...
        self._mw.z_max_InputWidget.setRange(self._scanning_logic.z_range[0],
                                            self._scanning_logic.z_range[1])
        self._mw.a_min_InputWidget.setRange(self._scanning_logic.a_range[0],
                                            self._scanning_logic.a_range[1])
        self._mw.a_max_InputWidget.setRange(self._scanning_logic.a_range[0],
                                            self._scanning_logic.a_range[1])
        ...
        # self._mw.a_min_InputWidget.setValue(self._scanning_logic.image_a_range[0])
        # self._mw.a_max_InputWidget.setValue(self._scanning_logic.image_a_range[1])
        ...
        if self.default_meter_prefix:
            ...
            self._mw.a_current_InputWidget.assumed_unit_prefix = self.default_meter_prefix
            ...
            self._mw.a_min_InputWidget.assumed_unit_prefix = self.default_meter_prefix
            self._mw.a_max_InputWidget.assumed_unit_prefix = self.default_meter_prefix
        ...
        self._mw.a_SliderWidget.sliderMoved.connect(self.update_from_slider_a)
        ...
        self._mw.a_res_InputWidget.setValue(self._scanning_logic.a_resolution)
        ...
        self._mw.a_current_InputWidget.editingFinished.connect(self.update_from_input_a)
        ...
        self._mw.a_res_InputWidget.editingFinished.connect(self.change_a_resolution)
        ...
        # self._mw.a_min_InputWidget.editingFinished.connect(self.change_a_image_range)
        # self._mw.a_max_InputWidget.editingFinished.connect(self.change_a_image_range)
        ...
        # self._scanning_logic.signal_history_event.connect(self.change_a_image_range)
...
#        self._mw.actionRotated_depth_scan.setEnabled(True)
    ...
    def update_from_input_a(self):
        """ The user changed the number in the z position spin box, adjust all
           other GUI elements."""
        a_pos = self._mw.a_current_InputWidget.value()
        self.update_roi_depth(v=a_pos)
        self.update_slider_a(a_pos)
        self._scanning_logic.set_position('ainput', a=a_pos)
        self._optimizer_logic.set_position('ainput', a=a_pos)
    ...
    def update_input_a(self, a_pos):
        """ Update the displayed z-value.

        @param float z_pos: the current value of the z position in m
        """
        # Convert x_pos to number of points for the slider:
        self._mw.a_current_InputWidget.setValue(a_pos)
    ...
    def update_from_slider_a(self, sliderValue):
        """The user moved the z position slider, adjust the other GUI elements.

        @params int sliderValue: slider postion, a quantized whole number
        """
        a_pos = self._scanning_logic.a_range[0] + sliderValue * self.slider_res
        # self.update_roi_depth(v=a_pos)
        self.update_input_a(a_pos)
        self._scanning_logic.set_position('aslider', a=a_pos)
        self._optimizer_logic.set_position('aslider', a=a_pos)
    ...
    def update_slider_a(self, a_pos):
        """ Update the a slider when a change happens.

        @param float a_pos: a position in m
        """
        self._mw.a_SliderWidget.setValue((a_pos - self._scanning_logic.a_range[0]) / self.slider_res)
    ...
    def change_a_resolution(self):
        """ Update the z resolution in the logic according to the GUI.
        """
        self._scanning_logic.a_resolution = self._mw.a_res_InputWidget.value()
    ...
    #
    # def change_a_image_range(self):
    #     """ Adjust the image range for z in the logic. """
    #     self._scanning_logic.image_a_range = [
    #         self._mw.a_min_InputWidget.value(),
    #         self._mw.a_max_InputWidget.value()]
